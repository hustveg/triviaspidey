// data/questions.js
// Banco de preguntas de trivia de Spider-Man.
// IMPORTANTE: este archivo NO esta dentro de /public a proposito.
// Si estuviera en /public, cualquier jugador podria abrir la URL
// directamente en el navegador y ver "correctIndex" antes de jugar.
//
// Sobre las imagenes: no se incluyen imagenes de Spider-Man, Marvel,
// peliculas o actores porque son propiedad de Disney/Marvel/los estudios
// (no son de uso libre aunque aparezcan en una busqueda). Todas quedan
// con image: null, lo que muestra automaticamente el placeholder con
// textura de telarana. Si quieres agregar tus propias imagenes (fotos
// libres de arañas, ciudad de noche, etc.), ponlas en
// public/assets/images/ y cambia el campo "image" de la pregunta.
//
// Preguntas 1-9: traducidas del ingles / mejoradas, con respuestas
// investigadas y verificadas (Wikipedia, Marvel.com, Britannica).
// Preguntas 10-19: tal como las enviaste, ya traian su respuesta correcta.
//
// NOTA: se omitio la pregunta "which villain claims to have the
// proportional strength of a spider" porque no se pudo verificar un
// villano concreto con esa caracteristica exacta en ninguna fuente
// confiable. Si me confirmas de donde la sacaste, la agrego despues.

module.exports = [
  {
    id: 1,
    question: "¿Quién creó a Spider-Man?",
    image: "assets/images/q1.jpg",
    options: [
      "Stan Lee y Steve Ditko",
      "Stan Lee y Jack Kirby",
      "Bob Kane",
      "Jim Lee y Frank Miller"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 2,
    question: "¿En qué año apareció Spider-Man por primera vez en los cómics?",
    image: "assets/images/q2.webp",
    options: [
      "1962",
      "1965",
      "1959",
      "1970"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 3,
    question: "¿Cuál es el verdadero nombre de Spider-Man?",
    image: "assets/images/q3.jpg",
    options: [
      "Peter Parker",
      "Harry Osborn",
      "Miles Morales",
      "Eddie Brock"
    ],
    correctIndex: 0,
    timeLimit: 15
  },
  {
    id: 4,
    question: "¿Cómo se llama el clon más famoso de Spider-Man?",
    image: "assets/images/q4.jpg",
    options: [
      "Ben Reilly",
      "Miles Morales",
      "Eddie Brock",
      "Otto Octavius"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 5,
    question: "¿En qué película murió Spider-Man (Peter Parker)?",
    image: "assets/images/q5.jpg",
    options: [
      "Spider-Man: Un Nuevo Universo",
      "Spider-Man 3",
      "The Amazing Spider-Man 2",
      "Spider-Man: No Way Home"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 6,
    question: "¿En qué año se estrenó la primera película de Spider-Man en el cine?",
    image: "assets/images/q6.jpg",
    options: [
      "2002",
      "2000",
      "2004",
      "1998"
    ],
    correctIndex: 0,
    timeLimit: 15
  },
  {
    id: 7,
    question: "¿Quién dirigió la primera película de Spider-Man en el cine (2002)?",
    image: "assets/images/q7.jpg",
    options: [
      "Sam Raimi",
      "Marc Webb",
      "Jon Watts",
      "Christopher Nolan"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 8,
    question: "¿Quién es el actor principal de The Amazing Spider-Man?",
    image: "assets/images/q8.webp",
    options: [
      "Andrew Garfield",
      "Tobey Maguire",
      "Tom Holland",
      "Jake Gyllenhaal"
    ],
    correctIndex: 0,
    timeLimit: 15
  },
  {
    id: 9,
    question: "¿Cuál de estos villanos nunca apareció en la serie animada de los 90?",
    image: "assets/images/q9.webp",
    options: [
      "Mister Negative",
      "Morbius",
      "Hydro-Man",
      "Scorpion"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 10,
    question: "En la serie animada de los 90, ¿quién es el principal enemigo del arco final?",
    image: "assets/images/q10.webp",
    options: [
      "Spider-Carnage",
      "Venom",
      "Kingpin",
      "Hobgoblin"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 11,
    question: '¿Qué villano dice: "El poder del sol en la palma de mi mano"?',
    image: "assets/images/q11.webp",
    options: [
      "Doctor Octopus",
      "Electro",
      "Norman Osborn",
      "Mysterio"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 12,
    question: "¿Cómo se llama la inteligencia artificial del traje de Tom Holland?",
    image: "assets/images/q12.webp",
    options: [
      "Karen",
      "Friday",
      "Edith",
      "May"
    ],
    correctIndex: 0,
    timeLimit: 15
  },
  {
    id: 13,
    question: "¿Quién entrenó a Spider-Man en artes marciales después de perder su sentido arácnido?",
    image: "assets/images/q13.jpg",
    options: [
      "Shang-Chi",
      "Iron Fist",
      "Daredevil",
      "Capitán América"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 14,
    question: "¿Cuál fue el primer villano en descubrir la identidad secreta de Peter Parker?",
    image: "assets/images/q14.jpg",
    options: [
      "Green Goblin",
      "Doctor Octopus",
      "Venom",
      "Kingpin"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 15,
    question: "¿Quién fue el primer supervillano de Spider-Man en los cómics?",
    image: "assets/images/q15.jpg",
    options: [
      "Chameleon",
      "Doctor Octopus",
      "Green Goblin",
      "Vulture"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 16,
    question: "¿Qué Spider-Man es considerado el más poderoso de todo el Spider-Verso?",
    image: "assets/images/q16.webp",
    options: [
      "Cosmic Spider-Man",
      "Spider-Ham",
      "Spider-Noir",
      "Spider-Punk"
    ],
    correctIndex: 0,
    timeLimit: 25
  },
  {
    id: 17,
    question: "¿Qué científico creó el simbionte Venom?",
    image: "assets/images/q17.jpg",
    options: [
      "Nadie. Es una especie alienígena",
      "Norman Osborn",
      "Reed Richards",
      "Otto Octavius"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 18,
    question: "¿Quién fue la primera novia formal de Peter Parker?",
    image: "assets/images/q18.webp",
    options: [
      "Gwen Stacy",
      "Mary Jane",
      "Liz Allan",
      "Felicia Hardy"
    ],
    correctIndex: 0,
    timeLimit: 20
  },
  {
    id: 19,
    question: "¿Quiénes fueron los primeros héroes en ayudar a Spider-Man?",
    image: "assets/images/q19.webp",
    options: [
      "Los Cuatro Fantásticos",
      "Los X-Men",
      "Deadpool",
      "Los Vengadores"
    ],
    correctIndex: 0,
    timeLimit: 20
  }
];
